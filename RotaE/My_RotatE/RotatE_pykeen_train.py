...
# Paths to your dataset files (adjust as needed)
...
# Step 1: Load the triples factories
# Training set
training = TriplesFactory.from_path(TRAIN_PATH)

# Validation set (share entity and relation mappings from training)
...
# Step 2: Train the RotatE model using the pipeline
result = pipeline(
    training=training,
    testing=validation,
    model='RotatE',
    model_kwargs=dict(embedding_dim=256),  # Adjust dimension as needed
    training_kwargs=dict(num_epochs=100, batch_size=128),  # Tune these
    optimizer='adam',
